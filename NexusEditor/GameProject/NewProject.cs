...
{
    private readonly string _templatePath = @"..\..\NexusEditor\ProjectTemplates";
    ...
    private string _projectPath = $@"{Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments)}\NexusProjects\";
    ...
    public string CreateProject(ProjectTemplate template)
    {
        ...
        try
        {
            ...
            {
                Directory.CreateDirectory(Path.Combine(Path.GetDirectoryName(path), folder));
            }

            var dirInfo = new DirectoryInfo(path + @".Nexus\");
        ...
        catch (Exception ex)
        {
            ...
            Logger.Log(MessageType.Error, $"创建项目失败 {ProjectName}");
            throw;
        }
    }
    ...
    public NewProject()
    {
        ...
        {
            Debug.WriteLine(ex.Message);
            Logger.Log(MessageType.Error, $"无法读取项目模板");
            throw;
        }
    }
}