...
class OpenProject
{
    private static readonly string _applicationDataPath = $@"{Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData)}\NexusEditor\";
    ...
    private static void ReadProjectData()
    {
        if (File.Exists(_projectDataPath))
        {
            ...
            foreach (var project in projects)
            {
                ...
                {
                    project.Icon = File.ReadAllBytes($@"{project.ProjectPath}\.Nexus\Icon.png");
                    project.Screenshot = File.ReadAllBytes($@"{project.ProjectPath}\.Nexus\Screenshot.png");
    ...
    static OpenProject()
    {
        ...
        catch (Exception ex)
        {
            ...
            Logger.Log(MessageType.Error, $"无法读取项目数据");
            throw;
        }
    }
}